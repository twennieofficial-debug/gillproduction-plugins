...
void GillCreativeProcessor::releaseResources(){engine.request(2);}
...
void GillCreativeProcessor::process(juce::AudioBuffer<float>&b,bool hostBypass){
    ...
    gill::creative::Transport transport;if(auto*head=getPlayHead())if(auto position=head->getPosition()){transport.playing=position->getIsPlaying();if(auto bpm=position->getBpm())if(std::isfinite(*bpm)&&*bpm>0)transport.bpm=*bpm;if(auto ppq=position->getPpqPosition())if(std::isfinite(*ppq)){transport.ppq=*ppq;transport.hasPPQ=true;}}
...
void GillCreativeProcessor::getStateInformation(juce::MemoryBlock&out){
    auto state=apvts.copyState();state.setProperty("schema",1,nullptr);state.setProperty("program",currentProgram.load(),nullptr);auto plan=engine.savedPlan();
    if(gill::creative::Engine::validPlan(plan)){
        juce::ValueTree model("MODEL");model.setProperty("origin",plan.originPPQ,nullptr);model.setProperty("beats",plan.durationBeats,nullptr);model.setProperty("seconds",plan.durationSeconds,nullptr);model.setProperty("bpm",plan.bpm,nullptr);model.setProperty("applied",engine.isApplied(),nullptr);
        ...
        bool captureGood=kind!=CreativeKind::Reply;
        if(kind==CreativeKind::Reply)if(const auto*bank=engine.capture(plan.capture)){const int frames=bank->used.load();const int role=bank->role.load();const unsigned epoch=bank->epoch.load();if(frames>1&&frames<=bank->capacity&&(role==2||role==3)&&epoch==plan.captureEpoch){
...
void GillCreativeProcessor::setStateInformation(const void*data,int bytes){
    if(!data||bytes<=0||bytes>96*1024*1024)return;auto xml=getXmlFromBinary(data,bytes);if(!xml||!xml->hasTagName(stateId(kind).toString()))return;auto state=juce::ValueTree::fromXml(*xml);if(!state.isValid())return;
    if(static_cast<int>(state.getProperty("schema",0))!=1)return;
    ...
    const auto model=state.getChildWithName("MODEL");gill::creative::Plan plan;bool commit=false;std::vector<float>audio;int frames=0;double rate=48000;
    if(model.isValid()){
        plan.originPPQ=static_cast<double>(model["origin"]);plan.durationBeats=static_cast<float>(model["beats"]);plan.durationSeconds=static_cast<float>(model["seconds"]);plan.bpm=static_cast<float>(model["bpm"]);commit=static_cast<bool>(model["applied"]);
        ...
        for(int i=0;i<gill::creative::wavePoints;++i){const float value=static_cast<float>(model["w"+juce::String(i)]);if(!std::isfinite(value))return;plan.waveform[i]=std::clamp(value,0.f,16.f);}if(!gill::creative::Engine::validPlan(plan))return;
        if(kind==CreativeKind::Reply){juce::MemoryBlock capture;if(!capture.fromBase64Encoding(model["audio"].toString())||capture.getSize()<12)return;juce::MemoryInputStream stream(capture,false);frames=stream.readInt();rate=stream.readDouble();if(frames<2||frames>48000*60||!std::isfinite(rate)||rate<8000||rate>48000||capture.getSize()!=12+static_cast<size_t>(frames)*8)return;audio.resize(static_cast<size_t>(frames)*2);for(auto&sample:audio){sample=stream.readFloat();if(!std::isfinite(sample)||std::abs(sample)>16)return;}}
    }
    if(!prepared){pendingState.replaceWith(data,static_cast<size_t>(bytes));}
    else if(model.isValid()){
        const std::lock_guard<std::mutex>lock(modelProducer);
        if(kind==CreativeKind::Reply){plan.capture=engine.importCapture(audio.data(),frames,rate);if(plan.capture<0)return;plan.captureEpoch=engine.capture(plan.capture)->epoch.load();}engine.postPlan(plan,commit);
    }
    else engine.request(7);
