...
void exportWorkflow(CreativeKind kind){
 auto p=std::make_unique<GillCreativeProcessor>(kind);Head head;p->setPlayHead(&head);setup(*p,8000,127);p->startLearn();process(*p,head,3,8000);p->stopLearn();process(*p,head,.01,8000,127,false);const auto plan=p->plan();
 juce::MemoryBlock immediateState;p->getStateInformation(immediateState);auto immediateXml=juce::AudioProcessor::getXmlFromBinary(immediateState.getData(),static_cast<int>(immediateState.getSize()));const auto immediateModel=immediateXml?juce::ValueTree::fromXml(*immediateXml).getChildWithName("MODEL"):juce::ValueTree{};
 check(immediateModel.isValid()&&juce::File(immediateModel["sourceFile"].toString()).existsAsFile()&&p->archive.ready(plan.sourceRevision),p->getName()+" immediate STOP/save waits for the completed source asset without losing future export");
 check(p->canRender(),p->getName()+" captured source becomes a durable render asset");juce::AudioFormatManager formats;formats.registerBasicFormats();std::unique_ptr<juce::AudioFormatReader>source(formats.createReaderFor(p->archive.sourceFile(plan.sourceRevision)));check(source&&source->lengthInSamples==24000&&source->usesFloatingPointData,p->getName()+" durable source is exact-duration floating-point WAV");
 if(source){juce::AudioBuffer<float>part(2,127);source->read(&part,0,127,1000,true,true);double error=0;for(int i=0;i<127;++i)error=std::max(error,static_cast<double>(std::abs(part.getSample(0,i)-vocal((1000+i)/8000.))));check(error<1.e-7,p->getName()+" source capture preserves audio samples",error);}
 for(bool fx:{true,false}){check(p->renderEffects(fx),p->getName()+" asynchronous FX/MIX render starts");for(int i=0;i<10000&&p->renderer.busy();++i)juce::Thread::sleep(5);const auto file=p->renderer.file();std::unique_ptr<juce::AudioFormatReader>render(formats.createReaderFor(file));check(p->renderer.status==2&&render&&render->lengthInSamples>=120000,p->getName()+" render creates drag-ready audio including the effect tail");if(render){juce::AudioBuffer<float>part(2,1024);double energy=0;float peak=0;for(std::int64_t offset=0;offset<render->lengthInSamples;offset+=1024){const int n=static_cast<int>(std::min<std::int64_t>(1024,render->lengthInSamples-offset));render->read(&part,0,n,offset,true,true);for(int ch=0;ch<2;++ch)for(int i=0;i<n;++i){const auto x=part.getSample(ch,i);energy+=x*x;peak=std::max(peak,std::abs(x));}}check(std::isfinite(energy)&&energy>1.e-6&&peak<=.981f,p->getName()+" rendered effect is audible, finite and has output headroom",energy);}}
 juce::MemoryBlock state;p->getStateInformation(state);check(state.getSize()<150000,p->getName()+" project state references durable capture without embedding a five-minute file",state.getSize());auto restored=std::make_unique<GillCreativeProcessor>(kind);restored->setStateInformation(state.getData(),static_cast<int>(state.getSize()));Head second;restored->setPlayHead(&second);setup(*restored,8000,127);process(*restored,second,.01,8000,127,false);check(restored->canRender(),p->getName()+" restored project can render without another learn pass");p->setPlayHead(nullptr);restored->setPlayHead(nullptr);
}
...
void checkRootBounds(juce::AudioProcessorEditor&editor,const juce::String&name){bool inside=true;for(auto*child:editor.getChildren())if(child->isVisible())inside&=editor.getLocalBounds().contains(child->getBounds());check(inside,name+" every visible root control fits the editor");juce::Component*fx=nullptr,*mix=nullptr,*drag=nullptr,*preset=nullptr;for(auto*child:editor.getChildren()){if(child->getComponentID()=="renderFx")fx=child;if(child->getComponentID()=="renderMix")mix=child;if(child->getComponentID()=="dragWav")drag=child;if(child->getComponentID()=="preset")preset=child;}check(fx&&mix&&drag&&preset&&!fx->getBounds().intersects(mix->getBounds())&&!mix->getBounds().intersects(drag->getBounds())&&!fx->getBounds().intersects(preset->getBounds())&&!drag->getBounds().intersects(preset->getBounds()),name+" export controls fit without overlapping presets");}
...
#else
 setenv("GILL_CREATIVE_AUDIO_ROOT",assets.getFullPathName().toRawUTF8(),1);
#endif
#if JUCE_MAC
 gillInitialiseMacTestApplication();
#endif
 juce::ScopedJuceInitialiser_GUI gui;for(auto kind:{CreativeKind::Phrase,CreativeKind::Director,CreativeKind::Reply}){basic(kind);learning(kind);exportWorkflow(kind);tick();ui(kind);}check(allocations.load()==0,"wrapper audio blocks and learning transitions allocate no heap",allocations.load());std::printf("RESULT %d checks %d failures\n",checks,failures);std::ofstream report("creative-integration-report.json");report<<"{\"checks\":"<<checks<<",\"failures\":"<<failures<<",\"audio_allocations\":"<<allocations.load()<<"}";return failures?1:0;}
