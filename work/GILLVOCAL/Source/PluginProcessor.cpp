...
:AudioProcessor(BusesProperties().withInput("INPUT",juce::AudioChannelSet::stereo(),true).withOutput("OUTPUT",juce::AudioChannelSet::stereo(),true)),kind(k),isLiveTune(k==GillKind::Tune&&liveTune),apvts(*this,nullptr,stateId(k),layout(k)) {
...
juce::AudioProcessorValueTreeState::ParameterLayout GillVocalProcessor::layout(GillKind k) {
...
void GillVocalProcessor::prepareToPlay(double fs,int block){
    ...
    updateParameters();
    for(auto& r:dryRing)r.assign(static_cast<size_t>(latency+1),0.f);dryPosition=0;
    ...
    inputPeak=0;outputPeak=0;reduction=0;pitchHz=0;targetHz=0;pitchConfidence=0;
}
...
void GillVocalProcessor::process(juce::AudioBuffer<float>& buffer,bool hostBypass){
    ...
    for(int start=0;start<count;start+=chunk){const int n=std::min(chunk,count-start);std::array<float*,2> ptr{};
        ...
        for(int i=0;i<n;++i){
            for(int c=0;c<channels;++c){const float raw=ptr[c][i];const float x=std::isfinite(raw)?juce::jlimit(-100.f,100.f,raw):0.f;ptr[c][i]=x;peakIn=std::max(peakIn,std::abs(x));
                ...
                else {dryRing[c][dryPosition]=x;dry[c][i]=dryRing[c][(dryPosition+1)%dryRing[c].size()];}}
