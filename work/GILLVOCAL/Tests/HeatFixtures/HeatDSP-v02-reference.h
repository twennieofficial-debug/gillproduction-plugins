...
// Existing 0..24 parameter range is retained for old projects. The upper half
// now opens progressively into extreme saturation, reaching 48 dB internally.
// Up to 12 the drive mapping is unchanged; its derivative is continuous there.
inline double effectiveDriveDb(double controlDb) noexcept {
    const double over = std::max(0.0, (controlDb - 12.0) / 12.0);
    return controlDb + 24.0 * over * over;
}
inline double driveNormalisation(double gain) noexcept {
    // Stop attenuating at 12 dB: extreme drive must remain audible, rather than
    // shrinking the already clipped waveform to nearly nothing.
    return std::min(gain, 3.9810717055349722);
}
    ...
    void process(float* const* buffers, int channels, int samples) noexcept {
        ...
        for (int sample = 0; sample < samples; ++sample) {
            ...
            for (std::size_t band = 0; band < gain_.size(); ++band) {
                ...
                // Introduce saturation continuously over the first 6 dB.
                amount[band] = std::clamp((gain_[band] - 1.0) /
                                          0.9952623149688795, 0.0, 1.0);
...
private:
    ...
    std::array<double, 3> gain_{{1.0, 1.0, 1.0}};
    std::array<double, 3> targetGain_{{1.0, 1.0, 1.0}};
