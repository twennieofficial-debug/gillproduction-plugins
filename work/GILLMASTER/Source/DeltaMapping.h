...
private:
 ...
 void cleanDeadMappings(const std::string& directory){
  ...
  while(auto* entry=readdir(dir)){if(++visited>2048||removed>=32)break;
   ...
   if(std::sscanf(entry->d_name,".gilldelta09_%llu_%d_%llx%n",&uid,&pid,&start,&end)!=3||entry->d_name[end]||uid!=getuid()||pid<=0)continue;
 ...
 void open(){
  ...
  name="Local\\GillDelta09_"+hex(hash(user))+"_"+std::to_string(GetCurrentProcessId())+"_"+hex((uint64_t(c.dwHighDateTime)<<32)|c.dwLowDateTime);
 ...
 #endif
  name+=".gilldelta09_"+std::to_string(getuid())+"_"+std::to_string(getpid())+"_"+hex(birth);
