        ...
        for (int i = 0; i < static_cast<int>(items.size()); ++i) {
            ...
            if (!oneLocalException) {
                ...
                if (mode == 1) {
                    ...
                    if (item.name == "GILLLOW" || item.name == "GILLGLUE" || item.name == "GILLWIDTH" || item.name == "GILLPUNCH" || item.name == "GILLDELTA" || item.name == "GILLDELIVER") check(latency == 0, item.name + " PRO adds no audio delay");
