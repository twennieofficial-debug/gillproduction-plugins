...
int main(){
    ...
    {
        auto processorOwner=std::make_unique<GillAssistProcessor>();auto&p=*processorOwner;p.setPlayConfigDetails(2,2,48000,127);p.prepareToPlay(48000,127);check(p.getName()=="GILLASSIST"&&p.getNumPrograms()==6,"Product identity and six presets");
    ...
    {
        auto processorOwner=std::make_unique<GillAssistProcessor>();auto&p=*processorOwner;p.prepareToPlay(8000,128);p.setCurrentProgram(5);PlayHead ph;ph.playing=true;ph.samples=16000;p.setPlayHead(&ph);p.engine.arm();check(waitFor([&]{return p.engine.state==Engine::Armed;}),"Capture-state regression arms normally");
