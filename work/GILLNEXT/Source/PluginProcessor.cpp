...
double GillNextProcessor::getTailLengthSeconds()const{return impl->publishedTail.load(std::memory_order_relaxed);}
void GillNextProcessor::process(juce::AudioBuffer<float>&buffer,bool hostBypass){const int blockQuality=qualityClient.mode();juce::ScopedNoDenormals nd;auto&e=*impl;if(!e.prepared)return;auto main=getBusBuffer(buffer,false,0);const int channels=std::min(2,main.getNumChannels()),frames=main.getNumSamples();if(channels<1||frames<=0)return;e.update();
...
e.publishedTail.store(kind==NextKind::Form?e.form.tailSeconds():kind==NextKind::Finish?e.finish.tailSeconds():kind==NextKind::Pocket?e.pocket.tailSeconds():e.latency/e.fs,std::memory_order_relaxed);
    qualityTransition.process(buffer.getArrayOfWritePointers(),channels,frames,blockQuality,kind==NextKind::Clean||kind==NextKind::Form||kind==NextKind::Finish);
...
void GillNextProcessor::setStateInformation(const void*data,int bytes){if(!data||bytes<=0||bytes>1024*1024)return;if(auto x=getXmlFromBinary(data,bytes))if(x->hasTagName(apvts.state.getType())){auto incoming=juce::ValueTree::fromXml(*x),clean=apvts.copyState();
...
std::vector<std::pair<juce::RangedAudioParameter*,float>>accepted;
