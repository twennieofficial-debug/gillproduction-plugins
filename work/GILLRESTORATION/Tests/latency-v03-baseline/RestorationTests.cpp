...
void exactAndBlockTests(){
    for(auto mode:{Mode::Declick,Mode::Decrackle})for(double fs:{44100.0,48000.0,88200.0,96000.0,192000.0}){
        RestorationEngine e;e.prepare(fs,mode);e.setAmount(0);const int delay=e.getLatencySamples();require(delay==static_cast<int>(std::ceil(fs*(mode==Mode::Declick?.004:.008))),"reported fixed latency");
