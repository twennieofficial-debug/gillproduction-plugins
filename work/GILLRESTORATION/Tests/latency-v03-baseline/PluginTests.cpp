...
void png(juce::AudioProcessorEditor& editor,int size,const juce::String& name){editor.setSize(size,size);auto img=editor.createComponentSnapshot(editor.getLocalBounds());juce::FileOutputStream out(juce::File::getCurrentWorkingDirectory().getChildFile(name+"-UI-"+juce::String(size)+".png"));check(out.openedOk(),"snapshot file opens");if(!out.openedOk())return;out.setPosition(0);out.truncate();juce::PNGImageFormat format;check(format.writeImageToStream(img,out),"snapshot writes");}
template<class T>void dryRoutes(gillrestoration::Mode mode,double fs,int channels){
    const int delay=static_cast<int>(std::ceil(fs*(mode==gillrestoration::Mode::Declick?.004:.008)));
...
void run(gillrestoration::Mode mode){
    ...
    if(editor){std::vector<juce::Slider*> sliders;std::vector<juce::Button*> buttons;collect(*editor,sliders,buttons);
        ...
        check(editor->getWidth()==420&&editor->getHeight()==420,"420 square default");auto* limit=editor->getConstrainer();
        check(limit&&limit->getMinimumWidth()==340&&limit->getMinimumHeight()==340&&limit->getMaximumWidth()==840&&limit->getMaximumHeight()==840&&limit->getFixedAspectRatio()==1.,"340-840 square resizing");
        if(sliders.size()==1){auto* slider=sliders[0];check(slider->getName()=="AMOUNT","single named Amount knob");
            ...
            set(p,"amount",55);for(int size:{340,420,840}){
                ...
                check(label&&slider->getLocalBounds().contains(label->getBounds())&&label->getWidth()>=150,"editable percentage remains legible inside the knob component");
            }
        }
        editor.reset();}
    p.releaseResources();
