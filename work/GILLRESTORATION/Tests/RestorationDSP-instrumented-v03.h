...
public:
    void prepare(double fs,Mode selectedMode) {
        ...
        const int previousLatency=static_cast<int>(std::ceil(sampleRate*(mode==Mode::Declick?0.004:0.008)));
        ...
        // Preserve every original context point while removing unused FIFO time.
        // updateContext's farthest right point is t+maximumGap+41*stride,
        // and its curvature reads one sample beyond that. The extra two keep
        // all reads strictly before the current write position. At low rates,
        // retain the previous clamp and therefore exactly the same context.
        latency=std::min(previousLatency,maximumGap+41*stride+2);
...
private:
    ...
    static double raw(const Channel& c,std::int64_t n) noexcept { latency_probe::read(n); return n<0?0.0:c.raw[static_cast<size_t>(index(n))]; }
    ...
    template<class T> void processImpl(T** data,int nChannels,int nSamples,T** delayedDry) noexcept {
        ...
        for(int i=0;i<nSamples;++i,++position){ latency_probe::write(position);
