...
int impulseMain(int argc,char** argv) {
    ...
    std::vector<int> reportedDelays,measuredDelays;
    ...
    for(double fs:{44100.,48000.,96000.,192000.}) {
        ...
        const int previous=static_cast<int>(std::ceil(fs*(name=="GILLDECLICK"?.004:.008)));
        const int gap=std::max(2,static_cast<int>(std::ceil(fs*(name=="GILLDECLICK"?.00065:.00035))));
        const int expected=(name=="GILLEQ"||name=="GILL-DE-ESSER")?0:std::min(previous,gap+41*std::max(1,static_cast<int>(std::lround(fs/48000.0)))+2);
        ...
        reportedDelays.push_back(plugin->getLatencySamples());measuredDelays.push_back(peakPosition);
        std::cout<<name<<" PDC fs="<<fs<<" reported="<<plugin->getLatencySamples()<<" measured="<<peakPosition<<" amplitude="<<peak<<'\n';
    ...
    std::ofstream report(destination.getFullPathName().toStdString());report<<"{\"passed\":"<<(failures?"false":"true")<<",\"checks\":"<<checks<<",\"failures\":"<<failures<<",\"name\":\""<<name<<"\",\"version\":\""<<types[0]->version<<"\",\"sample_rates\":[44100,48000,96000,192000],\"reported_latency_samples\":[";
    for(size_t i=0;i<reportedDelays.size();++i)report<<(i?",":"")<<reportedDelays[i];report<<"],\"measured_impulse_samples\":[";
    for(size_t i=0;i<measuredDelays.size();++i)report<<(i?",":"")<<measuredDelays[i];report<<"]}\n";
