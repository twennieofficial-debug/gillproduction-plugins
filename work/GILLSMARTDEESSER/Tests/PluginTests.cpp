...
int main(int argc,char** argv){std::cout<<std::unitbuf;
 ...
 juce::ScopedJuceInitialiser_GUI gui;using test::check;
 {
    ...
    auto vocal=test::vocal(48000);p.startLearning();feed(p,vocal);gillsmart::Profile learned;check(p.learnedCandidate(learned),"real active capture produces a candidate");check(p.value("profile")==0&&p.value("frequency")==6500,"learn waits for APPLY before altering sound");
    ...
    p.startLearning();feed(p,vocal);check(p.applyLearned(),"second measured profile can be applied");
