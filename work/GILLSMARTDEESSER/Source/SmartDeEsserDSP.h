#pragma once
// Original GILL DSP, isolated automatic de-esser extension. The public RBJ constant-peak band-pass
...
inline BandCoefficients designBand(double hz,double fs,double q=bandQ) noexcept {
    fs=validSampleRate(fs);hz=effectiveFrequency(hz,fs);
    const double w=2*pi*hz/fs,alpha=std::sin(w)/(2*clampFinite(q,.6,2.5,.8)),a0=1+alpha;
...
public:
    ...
    void reset() noexcept {
        ...
        q.reset(requestedQ);threshold.reset(requestedThreshold);maximum.reset(requestedMaximum);
        amount.reset(requestedAmount);logFrequency.reset(std::log(effectiveFrequency(requestedFrequency,sampleRate)));
        coefficients=designBand(std::exp(logFrequency.value),sampleRate,q.value);coefficientCountdown=0;
        updatePredictorLag();
    ...
    void setFrequency(double hz) noexcept {
        ...
        if(processed)logFrequency.set(target,parameterRampSamples);else {logFrequency.reset(target);coefficients=designBand(std::exp(target),sampleRate,q.value);updatePredictorLag();}
    }
    void setProfile(double hz,double bandwidth,double level,double maxCut) noexcept {
        setFrequency(hz);
        requestedQ=clampFinite(bandwidth,.6,2.5,.8);
        requestedThreshold=clampFinite(level,-72,-6,-48);
        requestedMaximum=clampFinite(maxCut,3,18,12);
        if(processed){q.set(requestedQ,parameterRampSamples);threshold.set(requestedThreshold,parameterRampSamples);maximum.set(requestedMaximum,parameterRampSamples);}
        else{q.reset(requestedQ);threshold.reset(requestedThreshold);maximum.reset(requestedMaximum);coefficients=designBand(std::exp(logFrequency.value),sampleRate,q.value);}
    }
...
private:
    ...
    double requestedQ=.8,requestedThreshold=-48,requestedMaximum=12;
    detail::Ramp q{.8,.8,0,0},threshold{-48,-48,0,0},maximum{12,12,0,0};
    ...
    double targetFor(const detail::Detector& d) const noexcept {
        ...
        const double relativeThreshold=.18-.11*amount.value;
        const double spectral=detail::smoothStep((ratio-relativeThreshold)/(.42-relativeThreshold));
        const double noise=detail::smoothStep((d.noisiness()-.015)/.16);
        const double level=10*std::log10(std::max(1e-16,d.bandPower));
        const double audible=detail::smoothStep((level-threshold.value)/9);
        return maximum.value*spectral*noise*audible;
    }
    template<class Sample> void processImpl(Sample** channels,int nChannels,int nSamples,Sample** listenBand) noexcept {
        ...
        for(int n=0;n<nSamples;++n) {
            amount.next();threshold.next();maximum.next();frequencyDirty=q.next()||frequencyDirty;frequencyDirty=logFrequency.next()||frequencyDirty;
            if(--coefficientCountdown<=0){coefficientCountdown=8;if(frequencyDirty){coefficients=designBand(std::exp(logFrequency.value),sampleRate,q.value);updatePredictorLag();frequencyDirty=false;}}
            ...
            reduction=std::clamp(target+decay*(reduction-target),0.0,18.0);
            ...
            for(int c=0;c<channelCount;++c) {
                ...
                if(listenBand&&listenBand[c])listenBand[c][n]=safeCast<Sample>(input[c]-output);
