...
struct GillDynamicsEditor::Impl:private juce::Timer {
    ...
    void resized(){const float scale=owner.getWidth()/static_cast<float>(width);canvas.setBounds(0,0,width,height);canvas.setTransform(juce::AffineTransform::scale(scale));
        if(p.kind==DynKind::Vox){quality.setBounds(185,62,110,26);button("bypass",325,207,80,28);place("comp",146,91,188,195);place("gate",39,230,106,108);place("output",335,239,106,108);presetAt(128,339,224);}
        else if(p.kind==DynKind::Opta){quality.setBounds(140,51,110,26);optical.setBounds(110,83,170,170);button("bypass",73,260,97,28);button("noise",220,260,99,28);place("gain",78,299,109,126);place("hf",233,297,83,80);place("reduction",216,378,126,130);choice("limit",62,428,126);choice("meter",61,480,148);presetAt(95,530,200);}
        ...
        else{quality.setBounds(195,20,110,26);map.setBounds(99,47,302,302);button("bypass",394,221,82,28);place("doubler",88,355,77,95);place("spread",170,355,77,95);place("mix",252,355,77,95);place("output",334,355,77,95);button("direct",108,455,137,28);button("mono",255,455,137,28);presetAt(132,489,236);}
    }
