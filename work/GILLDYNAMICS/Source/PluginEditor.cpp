...
struct GillDynamicsEditor::Impl:private juce::Timer {
    ...
    void paint(juce::Graphics&g){
        ...
        else{logo(g,20,17,40);text(g,"GILLSTAGE",{74,16,186,29},20);juce::Graphics::ScopedSaveState saved(g);g.reduceClipRegion(silhouette());juce::Path pod;pod.startNewSubPath(55,349);pod.cubicTo(81,324,104,331,126,348);pod.cubicTo(188,380,312,380,374,348);pod.cubicTo(396,331,419,324,445,349);pod.cubicTo(469,392,440,456,412,481);pod.cubicTo(348,503,153,503,88,481);pod.cubicTo(60,456,31,392,55,349);pod.closeSubPath();
            for(int i=5;i>=1;--i){g.setColour(juce::Colours::black.withAlpha(.075f));g.fillPath(pod,juce::AffineTransform::translation(0,i*.9f));}
            juce::ColourGradient face(cream.interpolatedWith(juce::Colours::white,.55f),70,333,cream.darker(.22f),110,500,false);face.addColour(.22,cream.brighter(.04f));face.addColour(.72,cream);g.setGradientFill(face);g.fillPath(pod);
            {juce::Graphics::ScopedSaveState surface(g);g.reduceClipRegion(pod);g.setTiledImageFill(gill::material::enamelGrain(),40,330,.4f);g.fillRect(pod.getBounds());g.setGradientFill(juce::ColourGradient(juce::Colours::white.withAlpha(.24f),120,330,juce::Colours::transparentWhite,430,488,true));g.fillRect(pod.getBounds());}
            g.setColour(ink.withAlpha(.32f));g.strokePath(pod,juce::PathStrokeType(2.6f));g.setGradientFill(juce::ColourGradient(juce::Colours::white.withAlpha(.9f),70,333,cream.darker(.28f),70,499,false));g.strokePath(pod,juce::PathStrokeType(1.1f));}
