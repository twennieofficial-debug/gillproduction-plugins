...
void disc(juce::Graphics& g,juce::Rectangle<float> r){g.setColour(juce::Colours::black.withAlpha(.2f));g.fillEllipse(r.translated(1,4));g.setGradientFill(juce::ColourGradient(juce::Colour(0xfffffbf0),r.getX(),r.getY(),juce::Colour(0xffc9beaa),r.getRight(),r.getBottom(),false));g.fillEllipse(r);g.setColour(cream);g.drawEllipse(r.reduced(1.5f),1.4f);g.setColour(ink.withAlpha(.25f));g.drawEllipse(r,1.f);}
...
public:
    ...
    void drawRotarySlider(juce::Graphics& g,int x,int y,int w,int h,float value,float start,float end,juce::Slider& s)override{
        const float size=std::min(static_cast<float>(w),static_cast<float>(h))-12.f,radius=size*.5f,cx=x+w*.5f,cy=y+h*.5f,angle=start+value*(end-start);auto body=juce::Rectangle<float>(size*.85f,size*.85f).withCentre({cx,cy});
        juce::Path rail,active;rail.addCentredArc(cx,cy,radius,radius,0,start,end,true);active.addCentredArc(cx,cy,radius,radius,0,start,angle,true);g.setColour(ink.withAlpha(.22f));g.strokePath(rail,juce::PathStrokeType(size<60?2.f:4.f));g.setColour(sage);g.strokePath(active,juce::PathStrokeType(size<60?3.f:5.f,juce::PathStrokeType::curved,juce::PathStrokeType::rounded));
        if(size>70){g.setColour(ink.withAlpha(.6f));for(int i=0;i<=20;++i){const float a=start+(end-start)*i/20.f;g.drawLine(cx+std::sin(a)*(radius+3),cy-std::cos(a)*(radius+3),cx+std::sin(a)*(radius+6),cy-std::cos(a)*(radius+6),.9f);}}
        disc(g,body);g.setColour(dark);g.drawLine(cx+std::sin(angle)*radius*.47f,cy-std::cos(angle)*radius*.47f,cx+std::sin(angle)*radius*.72f,cy-std::cos(angle)*radius*.72f,size>180?8.f:size>65?5.f:2.6f);
...
struct GillDynamicsEditor::Impl:private juce::Timer {
    ...
    Impl(GillDynamicsEditor& editor,GillDynamicsProcessor& processor):owner(editor),p(processor),look(processor),tooltip(&editor,650),graph(processor),map(processor),optical(processor,AnalogMeter::Optical),inMeter(processor,AnalogMeter::Input),outMeter(processor,AnalogMeter::Output){
        owner.setLookAndFeel(&look);owner.setOpaque(true);owner.addAndMakeVisible(canvas);canvas.setInterceptsMouseClicks(false,true);wood=juce::ImageCache::getFromMemory(BinaryData::core_reference_png,BinaryData::core_reference_pngSize);logoMask=extractLogo(wood);
    ...
    void resized(){const float scale=owner.getWidth()/static_cast<float>(width);canvas.setBounds(0,0,width,height);canvas.setTransform(juce::AffineTransform::scale(scale));
        if(p.kind==DynKind::Vox){button("bypass",325,207,80,28);place("comp",146,69,188,217);place("gate",39,230,106,108);place("output",335,239,106,108);presetAt(128,339,224);}
        else if(p.kind==DynKind::Opta){optical.setBounds(98,54,195,195);button("bypass",73,260,97,28);button("noise",220,260,99,28);place("gain",78,299,109,126);place("hf",233,297,83,80);place("reduction",216,378,126,130);choice("limit",62,428,126);choice("meter",61,480,148);presetAt(95,530,200);}
        else if(p.kind==DynKind::Buss){button("bypass",654,23,94,29);place("drive",42,78,129,146);place("trim",629,78,129,146);inMeter.setBounds(212,77,176,100);outMeter.setBounds(401,77,176,100);choice("style",215,185,276);choice("group",520,185,105);button("noise",645,239,104,29);groupsToggle.setBounds(48,239,123,29);
        ...
        else if(p.kind==DynKind::Quad){button("bypass",781,23,96,29);graph.setBounds(23,94,854,114);place("cross1",281,45,152,46);place("cross2",443,45,152,46);place("cross3",605,45,152,46);
        ...
        else{map.setBounds(99,47,302,302);button("bypass",394,221,82,28);place("doubler",88,355,77,95);place("spread",170,355,77,95);place("mix",252,355,77,95);place("output",334,355,77,95);button("direct",108,455,137,28);button("mono",255,455,137,28);presetAt(132,489,236);}
    }
    ...
    juce::Path silhouette()const{juce::Path q;
        if(p.kind==DynKind::Vox){q.startNewSubPath(240,8);q.cubicTo(147,8,112,61,104,138);q.cubicTo(101,185,87,193,55,214);q.cubicTo(2,245,9,324,43,350);q.cubicTo(83,376,122,362,153,360);q.cubicTo(185,376,297,376,328,360);q.cubicTo(372,378,433,370,460,336);q.cubicTo(491,289,461,231,421,211);q.cubicTo(381,192,379,158,375,134);q.cubicTo(365,54,319,8,240,8);q.closeSubPath();}
        else if(p.kind==DynKind::Opta){q.startNewSubPath(195,8);q.cubicTo(83,8,17,63,23,150);q.cubicTo(26,210,76,249,74,291);q.cubicTo(71,335,22,363,21,436);q.cubicTo(18,518,55,562,94,562);q.lineTo(296,562);q.cubicTo(338,562,374,517,368,437);q.cubicTo(363,363,320,331,317,290);q.cubicTo(315,248,367,211,367,147);q.cubicTo(367,62,305,8,195,8);q.closeSubPath();}
        else if(p.kind==DynKind::Buss){const float h=static_cast<float>(height);q.startNewSubPath(67,8);q.lineTo(733,8);q.cubicTo(771,8,765,89,778,154);q.cubicTo(790,212,793,h-64,780,h-33);q.cubicTo(774,h-10,744,h-8,704,h-8);q.lineTo(96,h-8);q.cubicTo(51,h-8,23,h-13,18,h-39);q.cubicTo(8,h-90,17,211,25,153);q.cubicTo(35,89,32,8,67,8);q.closeSubPath();}
        else if(p.kind==DynKind::Quad){q.startNewSubPath(64,10);q.cubicTo(196,-1,244,9,326,27);q.cubicTo(399,44,452,43,528,25);q.cubicTo(620,3,770,4,832,11);q.cubicTo(878,16,892,47,891,102);q.lineTo(891,474);q.cubicTo(892,517,870,539,820,539);q.cubicTo(718,539,642,533,535,542);q.cubicTo(392,555,292,541,230,539);q.lineTo(68,539);q.cubicTo(24,539,9,516,9,474);q.lineTo(9,102);q.cubicTo(9,48,20,18,64,10);q.closeSubPath();}
        else{q.startNewSubPath(250,8);q.cubicTo(113,8,8,126,8,269);q.cubicTo(8,405,51,492,112,519);q.cubicTo(173,537,327,537,388,519);q.cubicTo(449,492,492,405,492,269);q.cubicTo(492,126,387,8,250,8);q.closeSubPath();}return q;
    }
    bool hitTest(int x,int y)const{const float scale=owner.getWidth()/static_cast<float>(width);if(x>=owner.getWidth()-18&&y>=owner.getHeight()-18)return true;return silhouette().contains(x/scale,y/scale);}
    void woodBody(juce::Graphics&g,const juce::Path&q){juce::DropShadow(juce::Colours::black.withAlpha(.35f),6,{0,2}).drawForPath(g,q);g.setColour(juce::Colour(0xffb9895d));g.fillPath(q);{juce::Graphics::ScopedSaveState save(g);g.reduceClipRegion(q);if(wood.isValid())g.drawImage(wood,0,0,width,height,950,285,170,805);g.setColour(p.kind==DynKind::Stage?juce::Colour(0xff6c3f23).withAlpha(.34f):p.kind==DynKind::Buss?juce::Colour(0xff784a2e).withAlpha(.26f):p.kind==DynKind::Vox?juce::Colour(0xffc27a43).withAlpha(.10f):cream.withAlpha(.13f));g.fillRect(0,0,width,height);}g.setColour(juce::Colour(0xff715135).withAlpha(.9f));g.strokePath(q,juce::PathStrokeType(3));g.setColour(cream.withAlpha(.65f));g.strokePath(q,juce::PathStrokeType(1));}
    ...
    void paint(juce::Graphics&g){
        // The VST3 host owns its rectangular parent. This restrained surround
        // does not claim native window transparency or alter FL Studio's HWND.
