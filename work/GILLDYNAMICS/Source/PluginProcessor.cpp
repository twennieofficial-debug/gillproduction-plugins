...
void GillDynamicsProcessor::prepareToPlay(double rate,int block){const bool good=std::isfinite(rate)&&rate>=8000&&rate<=192000;rateSupported=good;const auto fs=good?rate:48000.;uiRate=fs;const int ch=juce::jlimit(1,2,getTotalNumOutputChannels());vox.setLiveMode(false);buss.setLiveMode(false);update();latency=0;
 ...
 update();for(auto&r:dry)r.assign(static_cast<size_t>(latency+1),0);dryPosition=0;vox.setLiveMode(!qualityClient.isPro());buss.setLiveMode(!qualityClient.isPro());latency=kind==DynKind::Vox?vox.latencySamples():kind==DynKind::Buss?buss.latencySamples():0;setLatencySamples(latency);bypassFade.reset(fs,.005);bypassFade.setCurrentAndTargetValue(!good||at("bypass")>.5f?1.f:0.f);inputPeak=outputPeak=inputRms=outputRms=reduction=0;inPower=outPower=0;channelPower.fill(0);for(auto&r:channelOutputRms)r=0;for(auto&r:channelOutputPeak)r=0;meterAlpha=1-std::exp(-1/(fs*.3));qualityTransition.prepare(fs,qualityClient.mode());graphCounter=0;updateGraph();}
...
void GillDynamicsProcessor::process(juce::AudioBuffer<float>&b,bool bypass){const int blockQuality=qualityClient.mode();juce::ScopedNoDenormals nd;const int ch=std::min(2,b.getNumChannels()),n=b.getNumSamples();if(ch==0||n==0)return;for(int c=getTotalNumInputChannels();c<b.getNumChannels();++c)b.clear(c,0,n);if(dry[0].empty())return;vox.setLiveMode(blockQuality==0);buss.setLiveMode(blockQuality==0);latency=kind==DynKind::Vox?vox.latencySamples():kind==DynKind::Buss?buss.latencySamples():0;qualityClient.requestLatencySamples(latency);update();bypassFade.setTargetValue(bypass||!rateSupported.load()||at("bypass")>.5f?1.f:0.f);float inPeak=0,outPeak=0;std::array<float,2> outPeaks{};
 for(int start=0;start<n;start+=128){const int count=std::min(128,n-start);std::array<float*,2>ptr{};std::array<std::array<float,128>,2>original{};std::array<double,128>inputPowers{};for(int c=0;c<ch;++c)ptr[c]=b.getWritePointer(c,start);
  for(int i=0;i<count;++i){for(int c=0;c<ch;++c){const float x=std::clamp(safe(ptr[c][i]),-32.f,32.f);ptr[c][i]=x;inputPowers[i]+=double(x)*x/ch;inPeak=std::max(inPeak,std::abs(x));dry[c][dryPosition]=x;original[c][i]=dry[c][(dryPosition+dry[c].size()-static_cast<size_t>(latency))%dry[c].size()];}dryPosition=(dryPosition+1)%dry[0].size();}
  ...
  for(int i=0;i<count;++i){const float bp=bypassFade.getNextValue();double power=0;for(int c=0;c<ch;++c){const float wet=safe(ptr[c][i]);ptr[c][i]=bp>=1?original[c][i]:wet+bp*(original[c][i]-wet);outPeak=std::max(outPeak,std::abs(ptr[c][i]));outPeaks[c]=std::max(outPeaks[c],std::abs(ptr[c][i]));const double v=double(ptr[c][i])*ptr[c][i];power+=v/ch;channelPower[c]+=meterAlpha*(v-channelPower[c]);}inPower+=meterAlpha*(inputPowers[i]-inPower);outPower+=meterAlpha*(power-outPower);}
    ...
    qualityTransition.process(b.getArrayOfWritePointers(),ch,n,blockQuality,kind==DynKind::Vox||kind==DynKind::Buss);
...
void GillDynamicsProcessor::setStateInformation(const void*data,int bytes){
 ...
 if(auto x=getXmlFromBinary(data,bytes))if(x->hasTagName(apvts.state.getType())){
  auto incoming=juce::ValueTree::fromXml(*x),clean=apvts.copyState();
...
bool changed=false;std::array<std::array<float,4>,8>restored{};std::array<std::array<bool,4>,8>restoreGroup{};
